...
function getPlayerDataID($playerid,$dbhandle) {
	...
	//Bliss Schema v0.26
	$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND s.unique_id=?");
	// Old HIVE Schema
...
function getPlayersWithItem($searchitem,$dbhandle) {
	...
	//Bliss Schema v0.26
	$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND (s.inventory LIKE ? OR s.backpack LIKE ?)");
...
function getTopPlayers($orderby,$dbhandle) {
	...
	switch ($orderby) {
		...
		case "zombie_kills":
			...
			$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND (DATEDIFF(last_updated,now()) > -7) ORDER BY s.zombie_kills DESC LIMIT 10");
			break;
		case "survivor_kills":
			...
			$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND (DATEDIFF(last_updated,now()) > -7) ORDER BY s.survivor_kills DESC LIMIT 10");
			break;
		case "bandit_kills":
			...
			$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND (DATEDIFF(last_updated,now()) > -7) ORDER BY s.bandit_kills DESC LIMIT 10");
			break;
		case "start_time":
			...
			$qry = $dbhandle->prepare("SELECT s.id, s.unique_id, p.name, s.zombie_kills, s.survivor_kills, s.bandit_kills, s.start_time, s.last_updated, s.worldspace FROM profile p LEFT JOIN survivor s ON p.unique_id = s.unique_id WHERE s.is_dead=0 AND (DATEDIFF(last_updated,now()) > -7) ORDER BY s.start_time DESC LIMIT 10");
			break;
		//default:
